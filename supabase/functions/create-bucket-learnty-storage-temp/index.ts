...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "application/epub+zip", "image/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for learnty-storage" ON storage.objects FOR SELECT USING (bucket_id = 'learnty-storage');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for learnty-storage" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'learnty-storage');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for learnty-storage" ON storage.objects FOR UPDATE USING (bucket_id = 'learnty-storage');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for learnty-storage" ON storage.objects FOR DELETE USING (bucket_id = 'learnty-storage');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'learnty-storage',
        public: true,
        allowed_mime_types: ["application/pdf", "application/epub+zip", "image/*"],
