...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "application/pdf", "text/plain", "application/epub+zip"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for learnty-files" ON storage.objects FOR SELECT USING (bucket_id = 'learnty-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for learnty-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'learnty-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for learnty-files" ON storage.objects FOR UPDATE USING (bucket_id = 'learnty-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for learnty-files" ON storage.objects FOR DELETE USING (bucket_id = 'learnty-files');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'learnty-files',
        public: true,
        allowed_mime_types: ["image/*", "application/pdf", "text/plain", "application/epub+zip"],
