...
const SlideTwo = () => (
    <div className="flex flex-col items-center justify-center gap-6 md:gap-10 w-full max-w-md px-6 animate-[slideInRight_0.6s_ease-out] text-center">
        ...
        <div className="relative flex items-center justify-center w-full">
            <div className="w-40 h-40 md:w-64 md:h-64 bg-[#451a03]/80 backdrop-blur-sm border border-amber-700/30 rounded-xl flex items-center justify-center relative shadow-2xl overflow-hidden">
    ...
    <div className="flex flex-col items-center justify-center gap-8 w-full max-w-md px-6 animate-[slideInRight_0.6s_ease-out] text-center">
        <div className="w-full">
            ...
            <div className="flex justify-center gap-3">
                <div className="px-3 py-1 rounded bg-red-950/30 border border-red-900/50 text-xs font-mono text-red-300">$$$ EXPENSIVE</div>
                <div className="px-3 py-1 rounded bg-red-950/30 border border-red-900/50 text-xs font-mono text-red-300">latency: HIGH</div>
            </div>
        </div>
            ...
            <div className="w-full h-full bg-[#1e293b] rounded-3xl border border-slate-700 p-4 md:p-8 flex flex-col justify-between shadow-2xl relative overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-4 flex-1 mb-6">
                    <div className="bg-[#0f172a] rounded-2xl flex flex-col items-center justify-center p-4 border border-slate-700/50 relative overflow-hidden group">
                        ...
                        <span className="text-yellow-400 font-bold text-sm">+500 XP</span>
                    </div>
                    <div className="bg-[#0f172a] rounded-2xl flex flex-col items-center justify-center p-4 border border-slate-700/50 relative overflow-hidden group">
                        <div className="absolute inset-0 bg-[#0d9488]/5 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                        <div className="w-8 h-8 rounded-full border-2 border-[#0d9488] flex items-center justify-center mb-2">
                            <CheckCircle className="w-4 h-4 text-[#0d9488]" />
                        </div>
                        <span className="text-[#0d9488] font-bold text-sm">Level Up</span>
                    </div>
                </div>
                <div className="bg-[#0f172a] rounded-xl p-4 border border-slate-700/50">
                    <div className="h-3 w-full bg-slate-800 rounded-full overflow-hidden mb-2 relative">
                        <div className="absolute top-0 left-0 h-full w-[80%] bg-gradient-to-r from-[#0d9488] to-[#2dd4bf] shadow-[0_0_10px_#0d9488]"></div>
                    </div>
                    <div className="flex justify-end">
                        <span className="text-xs font-mono text-slate-300">80% Retained</span>
                    </div>
                </div>
            </div>
        </div>
...
const Intro = () => {
    ...
    const startGame = () => navigate('/dashboard');
    ...
    return (
        <PageTransition>
            <div className="relative w-full h-[100dvh] flex flex-col overflow-hidden no-select"
                onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
                <GalaxyBackground />
                <header className="fixed top-0 left-0 w-full p-4 md:p-6 flex justify-between items-center z-50">
                    ...
                    <button onClick={startGame} className="text-xs md:text-sm text-slate-400 hover:text-white transition-colors font-medium">
                        Skip Intro
                    </button>
                </header>
