...
const ChatSidebar = ({ isOpen, onClose, onNewChat }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            {/* Drawer */}
            <div className="relative w-[85%] md:w-[75%] max-w-sm h-full bg-slate-900/95 border-l border-white/10 shadow-2xl sidebar-enter flex flex-col animate-[slideInRight_0.3s_ease-out] pt-safe pb-safe">
                <div className="p-6 flex items-center justify-between border-b border-white/5">
                    <h2 className="text-xl font-bold font-grotesk text-white">History</h2>
                    <button onClick={onClose} className="p-2 text-slate-400 hover:text-white active:scale-95">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                    {historyItems.map(item => (
                        <div key={item.id} className="group p-4 rounded-2xl bg-slate-800/40 border border-white/5 hover:bg-[#0d9488]/10 hover:border-[#0d9488]/30 transition-all cursor-pointer active:scale-95">
...
const Chat = () => {
    ...
    return (
        <PageTransition>
            <div className="relative w-full h-[100dvh] flex flex-col overflow-hidden bg-[#0f172a]">
                <GalaxyBackground />
                ...
                {/* --- HEADER (Flex Item, Sticky) --- */}
                <header className="flex-shrink-0 relative z-30 flex items-center justify-between px-4 md:px-6 py-3 md:py-6 bg-gradient-to-b from-[#0f172a] to-[#0f172a]/0 pt-safe">

                    <button
                        onClick={() => navigate('/dashboard')}
                        className="pointer-events-auto p-3 rounded-full hover:bg-white/10 transition-colors text-slate-300 hover:text-white backdrop-blur-sm border border-transparent hover:border-white/10 active:scale-95"
                    >
                        <ArrowLeft className="w-6 h-6" />
                    </button>

                    <div className="pointer-events-auto flex flex-col items-center cursor-pointer group active:scale-95 transition-transform" onClick={startNewChat}>
                        <div className="relative">
                            ...
                            <Brain className="w-8 h-8 md:w-10 md:h-10 text-[#2dd4bf] relative z-10 drop-shadow-[0_0_10px_rgba(45,212,191,0.8)] transition-transform group-hover:rotate-12" />
                        </div>
                ...
                <footer className="flex-shrink-0 p-3 md:p-6 relative z-20 bg-slate-900/80 backdrop-blur-xl border-t border-white/5 pb-safe">
                    <form onSubmit={handleSend} className="max-w-3xl mx-auto relative flex items-end gap-2 md:gap-3">
                        ...
                        <div className="flex-1 relative">
                            <input
                                ...
                                className="w-full h-12 pl-5 pr-4 bg-slate-800/50 border border-slate-700 rounded-2xl text-white placeholder-slate-500 text-base focus:outline-none focus:border-[#0d9488] focus:ring-1 focus:ring-[#0d9488] transition-all shadow-inner"
                            />
                        </div>
                        <button
                            type="submit"
